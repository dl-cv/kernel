...
imx296_get_pad_crop(struct imx296 *sensor, struct v4l2_subdev_pad_config *cfg,
...
imx296_get_pad_format(struct imx296 *sensor, struct v4l2_subdev_pad_config *cfg,
		      unsigned int pad, enum v4l2_subdev_format_whence which)
{
	switch (which) {
	case V4L2_SUBDEV_FORMAT_TRY:
				...
				struct v4l2_subdev_selection *sel)
{
	...
	mutex_lock(&sensor->mutex);
	crop = imx296_get_pad_crop(sensor, cfg, sel->pad, sel->which);
	format = imx296_get_pad_format(sensor, cfg, sel->pad, sel->which);
