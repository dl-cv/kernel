 ...
 *   XVS – each pulse is generated by enable→sleep→disable.
 *          RK3588 PWM starts its counter from 0 on enable, so the pulse
 *          appears immediately.  We MUST NOT try to modulate duty while
 *          the PWM stays enabled, because RK3588 double-buffers the duty
 *          register and a new value only takes effect at the next period
 *          boundary (~33 ms for 30 fps), which is far too slow.
 *
 * Frame timing (frame_ns) is derived in priority order from:
 *   1) xhs_period * VTS   (most accurate)
 *   2) DT xvs_pwm period  (DT encodes ~one frame)
 *   3) mode fps            (last resort)
 */

/* Retrieve a reliable period for a PWM: current state, then DT args. */
...
/*
 * Emit exactly one XVS pulse via enable → sleep → disable.
 *
 * On RK3588, pwm_apply_state(enabled=true) resets the counter to 0 and
 * the configured duty waveform starts immediately.  We disable before the
 * second period begins so that only a single pulse is output.
 */
static int imx415_pulse_xvs(struct imx415 *imx415)
{
	...
	/* Enable with the desired duty – counter starts from 0, pulse is immediate. */
	...
	/*
	 * Sleep long enough for the duty (active) portion to complete, but
	 * disable before the next period starts to avoid a second pulse.
	 */
	period_us = div_u64(state.period, 1000);
	wait_us = DIV_ROUND_UP(duty_ns, 1000) + 50;
	if (wait_us < 100)
		wait_us = 100;
	/* Never sleep past the period boundary. */
	if (period_us && wait_us >= period_us)
		wait_us = (period_us > 1) ? (u32)(period_us - 1) : 1;

	usleep_range(wait_us, wait_us + 50);

	/* Disable – stops counter, no more pulses. */
	state.duty_cycle = 0;
	state.enabled = false;
	return pwm_apply_state(imx415->xvs_pwm, &state);
}

/* Compute one-frame duration in nanoseconds. */
static u64 imx415_frame_ns(struct imx415 *imx415)
{
	u64 xhs_period, xvs_period, frame;
	u32 vts;

	/* Method 1: VTS lines × XHS period. */
	...
	/* Method 2: DT XVS period (one period ≈ one frame). */
	...
	return 33333333ULL; /* 30 fps fallback */
}
...
static int imx415_trigger_locked(struct imx415 *imx415, unsigned int pulses)
{
	...
	xhs_period_ns = imx415_pwm_period_ns(imx415->xhs_pwm);
	xvs_period_ns = imx415_pwm_period_ns(imx415->xvs_pwm);

	/*
	 * Ensure XHS is running.
	 * Keep XHS continuously enabled to avoid line-phase alignment issues.
	 */
	xhs_was_enabled = imx415->xhs_pwm_enabled;
	...
	for (i = 0; i < pulses; i++) {
		...
		if (ret)
			goto out_restore_xhs;
		imx415->last_trigger_ns = ktime_get_ns();

		/* Inter-pulse gap: one full frame period. */
		if (i + 1 < pulses && frame) {
...
out_restore_xhs:
	/* Restore previous XHS state for non-trigger paths. */
	if (!imx415_use_pwm_trigger(imx415) && !xhs_was_enabled && imx415->xhs_pwm)
		imx415_set_xhs_pwm(imx415, false);

	/* One short line per trigger for debugging. */
	dev_info(&imx415->client->dev,
		 "trigger pulses=%u frame_ns=%llu xhs_p=%lluns xvs_p=%lluns ret=%d\n",
		 pulses, frame, xhs_period_ns, xvs_period_ns, ret);
			     ...
			     const char *buf, size_t count)
{
	...
	/*
	 * echo 1 > trigger  →  1 XVS pulse  (single-frame trigger)
	 * echo 2 > trigger  →  2 XVS pulses (spaced by one frame interval)
	 */
